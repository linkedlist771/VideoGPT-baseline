    ...
    def __init__(
        ...
        if bilinear:
            ...
            if skip:
                ...
                self.conv = ConvLayer(
                    out_channels * 2, out_channels, bn=bn, motion=motion
                )
            else:
                self.up = nn.ConvTranspose2d(
                    in_channels, out_channels, kernel_size=factor, stride=factor
...
class MatrixPredictor3DConv(nn.Module):
    def __init__(self, hidden_len=64):
        ...
        self.conv_pre_1 = nn.Conv2d(
            hidden_len, hidden_len, kernel_size=3, stride=1, padding=1
        )
        ...
        self.conv1_1 = nn.Conv2d(
            hidden_len, self.unet_base, kernel_size=3, stride=2, padding=1
        )
        self.conv2_1 = nn.Conv2d(
            self.unet_base, self.unet_base * 2, kernel_size=3, stride=2, padding=1
        )

        self.conv3_1 = nn.Conv2d(
            self.unet_base * 3, self.unet_base, kernel_size=3, stride=1, padding=1
        )
        self.conv4_1 = nn.Conv2d(
            self.unet_base, self.hidden_len, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, x):
        ...
        ).contiguous()  # (batch * seq, c, h//2, w//2), seq = 1
        ...
        x_1 = x_1.view(
            -1, x_1.size(2), x_1.size(3), x_1.size(4)
        ).contiguous()  # (batch*1, c, h, w)
        x_3 = F.leaky_relu(
            self.bn3_1(
                self.conv3_1(
                    torch.cat((F.interpolate(x_2, scale_factor=(2, 2)), x_1), dim=1)
                )
            )
        )
        x = x.view(batch, -1, x.size(1), x.size(2), x.size(3))  # (batch, seq, 1, h, w)
        x = F.leaky_relu(
            self.bn4_1(self.conv4_1(F.interpolate(x_3, scale_factor=(2, 2))))
        )
        return x
...
class SimpleMatrixPredictor3DConv_direct(nn.Module):
    def __init__(self, T, hidden_len=64, image_pred=False, aft_seq_length=10):
        ...
        self.conv_translate = nn.Sequential(
            nn.Conv2d(
                self.unet_base * input_len,
                self.unet_base * self.fut_len,
                kernel_size=1,
                stride=1,
                padding=0,
            ),
    ...
    def forward(self, x):
        ...
        x_3 = F.leaky_relu(
            self.bn3_1(
                self.conv3_1(
                    torch.cat((F.interpolate(x_2, size=x_1.shape[2:]), x_1), dim=1)
                )
            )
        )
        x = x.view(batch, -1, x.size(1), x.size(2), x.size(3))  # (batch, seq, 1, h, w)
...
class PredictModel(nn.Module):
    def __init__(
        ...
        self.fuse_conv = nn.Conv2d(
            hidden_len * 2, hidden_len, kernel_size=3, padding=1, bias=False
        )
        if use_direct_predictor:
            self.predictor = SimpleMatrixPredictor3DConv_direct(
                T=T, hidden_len=hidden_len, aft_seq_length=aft_seq_length
            )
        else:
    ...
    def res_interpolate(self, in_tensor, template_tensor):
        ...
        out_tensor = F.interpolate(
            in_tensor, template_tensor.shape[-2:]
        )  # (BThw,target_h,target_w)
    ...
    def forward(self, matrix_seq, softmax=False, res=None):
        ...
        matrix_seq = matrix_seq.reshape(B * T * hw, self.mx_h, self.mx_w).unsqueeze(
            1
        )  # (BThw,1,h,w)
        ...
        if res is not None:
            ...
            in_tensor = (
                res.clone().reshape(B, hw // 4, emb.shape[1], -1).permute(0, 2, 1, 3)
            )

            res_tensor = (
                self.res_interpolate(in_tensor, template)
                .permute(0, 2, 1, 3)
                .reshape(emb.shape)
            )
