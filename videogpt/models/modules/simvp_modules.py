...
class HorNetSubBlock(HorBlock):
    ...
    def __init__(self, dim, mlp_ratio=4.0, drop_path=0.1, init_value=1e-6):
        super().__init__(
            dim, mlp_ratio=mlp_ratio, drop_path=drop_path, init_value=init_value
        )
        self.apply(self._init_weights)
...
class SwinSubBlock(SwinTransformerBlock):
    ...
    def __init__(
        ...
        window_size = (
            7 if input_resolution[0] % 7 == 0 else max(4, input_resolution[0] // 16)
        )
...
def UniformerSubBlock(
    ...
    if block_type == "Conv":
        return CBlock(
            dim=embed_dims, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path
        )
    else:
        return SABlock(
            dim=embed_dims,
