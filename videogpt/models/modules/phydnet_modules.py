...
class PhyCell(nn.Module):
    def __init__(
        self, input_shape, input_dim, F_hidden_dims, n_layers, kernel_size, device
    ):
    ...
    def initHidden(self, batch_size):
        ...
        for i in range(self.n_layers):
            self.H.append(
                torch.zeros(
                    batch_size, self.input_dim, self.input_shape[0], self.input_shape[1]
                ).to(self.device)
            )
...
class PhyD_ConvLSTM(nn.Module):
    def __init__(
        self, input_shape, input_dim, hidden_dims, n_layers, kernel_size, device
    ):
...
class PhyD_EncoderRNN(torch.nn.Module):
    def __init__(self, phycell, convcell, in_channel=1, patch_size=4):
        ...
        self.encoder_E = encoder_E(
            nc=in_channel, patch_size=patch_size
