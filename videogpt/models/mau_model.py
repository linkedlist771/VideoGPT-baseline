...
class MAU_Model(nn.Module):
    ...
    def __init__(self, num_layers, num_hidden, configs, **kwargs):
        ...
        encoder.add_module(
            name="encoder_t_conv{0}".format(-1),
            module=nn.Conv2d(
                in_channels=self.frame_channel,
        ...
        for i in range(n):
            encoder = nn.Sequential()
            encoder.add_module(
                name="encoder_t{0}".format(i),
                module=nn.Conv2d(
                    in_channels=self.num_hidden[0],
                    out_channels=self.num_hidden[0],
                    stride=(2, 2),
                    padding=(1, 1),
                    kernel_size=(3, 3),
            ...
            encoder.add_module(
                name="encoder_t_relu{0}".format(i), module=nn.LeakyReLU(0.2)
            )
            encoders.append(encoder)
        ...
        for i in range(n - 1):
            ...
            decoder.add_module(
                name="c_decoder{0}".format(i),
                module=nn.ConvTranspose2d(
                    in_channels=self.num_hidden[-1],
            ...
            decoder.add_module(
                name="c_decoder_relu{0}".format(i), module=nn.LeakyReLU(0.2)
            )
    ...
    def forward(self, frames_tensor, mask_true, **kwargs):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for i in range(self.tau):
                tmp_t.append(
                    torch.zeros([batch_size, in_channel, height, width]).to(device)
                )
                tmp_s.append(
                    torch.zeros([batch_size, in_channel, height, width]).to(device)
                )
        ...
        for t in range(self.configs.total_length - 1):
            ...
            if t == 0:
                for i in range(self.num_layers):
                    zeros = torch.zeros(
                        [batch_size, self.num_hidden[i], height, width]
                    ).to(device)
                    T_t.append(zeros)
            S_t = frames_feature
            for i in range(self.num_layers):
        ...
        next_frames = (
            torch.stack(next_frames, dim=0).permute(1, 0, 2, 3, 4).contiguous()
        )
