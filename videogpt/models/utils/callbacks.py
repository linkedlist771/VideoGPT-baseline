    ...
    def on_fit_start(self, trainer, pl_module):
        ...
        if trainer.global_rank == 0:
            ...
            logging.basicConfig(
                level=logging.INFO,
                filename=osp.join(
                    self.save_dir, "{}_{}.log".format(self.prefix, self.setup_time)
                ),
                filemode="a",
...
class EpochEndCallback(Callback):
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if hasattr(self, "avg_train_loss"):
            print_log(
                f"Epoch {trainer.current_epoch}: Lr: {lr:.7f} | Train Loss: {self.avg_train_loss:.7f} | Vali Loss: {avg_val_loss:.7f}"
            )
