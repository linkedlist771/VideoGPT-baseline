...
def sim_matrix_postprocess(similar_matrix):
    ...
    similar_matrix = similar_matrix.reshape(
        similar_matrix.shape[0], similar_matrix.shape[1], -1
    )
...
def sim_matrix_interpolate(in_matrix, ori_hw, target_hw):
    ...
    ori_matrix_half = F.interpolate(
        ori_matrix.reshape(-1, ori_h, ori_w).unsqueeze(1),
        (int(target_h), int(target_w)),
        mode="bilinear",
    ).squeeze(
        1
    )  # (BThw,target_h,target_w)
    new_matrix = F.interpolate(
        ori_matrix_half.reshape(B, T, ori_h, ori_w, target_h, target_w)
        .permute(0, 1, 4, 5, 2, 3)
        .reshape(-1, ori_h, ori_w)
        .unsqueeze(1),
        (int(target_h), int(target_w)),
        mode="bicubic",
    ).squeeze(
        1
    )  # (BT*targethw,target_h,target_w)
    new_matrix = (
        new_matrix.reshape(B, T, target_h, target_w, target_h, target_w)
        .permute(0, 1, 4, 5, 2, 3)
        .reshape(B, T, target_h * target_w, target_h * target_w)
    )
...
def cum_multiply(value_seq, cum_softmax=False, reverse=True):
    ...
    if (
        not reverse
    ):  # reverse means last element is the one multiplied most times,i.e. the reference is the last element:
...
class CosineAnnealingLR_Restart(_LRScheduler):
    def __init__(
        ...
        self.last_restart = 0
        print(
            "restart ratio: ", ratio, " T_period: ", T_period, " minimum lr: ", eta_min
        )
