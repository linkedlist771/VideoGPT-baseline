        ...
        for t, ax in enumerate(axes):
            if t == 0:
                ...
                if i < prev_frames:
                    ...
                    if use_rgb:
                        im = ax.imshow(
                            cv2.cvtColor(true[i - frames], cv2.COLOR_BGR2RGB)
                        )
                    else:
                ...
                if i < prev_frames:
                    ...
                    if use_rgb:
                        im = ax.imshow(
                            cv2.cvtColor(pred[i - frames], cv2.COLOR_BGR2RGB)
                        )
                    else:
...
def show_video_gif_single(data, out_path=None, use_rgb=False):
    """Generate GIF with a video sequence."""
    if len(data.shape) > 3:
        # Adjust axes to move the channel dimension
        data = data.swapaxes(1, 2).swapaxes(2, 3)
        # Clip the values to ensure they are within [0, 1] bounds
        data = np.clip(data, 0, 1)
        # Squeeze the singleton dimension and scale to 0-255 for grayscale
        data = np.squeeze(data) * 255
        # Convert data to uint8 for image processing
    ...
    if out_path:
        ...
        imageio.mimsave(out_path, images, format="GIF", fps=5)
...
def show_weather_bench(
    heatmap, src_img=None, cmap="GnBu", title=None, out_path=None, vis_channel=None
):
    ...
    for i in range(heatmap.shape[0]):
        vis_img = show_heatmap_on_image(
            src_img,
            heatmap[i, vis_channel, ...],
            use_rgb=False,
            colormap=get_mpl_colormap(cmap),
            image_weight=0.1,
            image_binary=True,
        )
        ret_img.append(vis_img[np.newaxis, :])
