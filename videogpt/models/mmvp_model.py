...
class MMVP_Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.mat_size = [
            [H // highres_scale, W // highres_scale],
            [H // lowres_scale, W // lowres_scale],
        ]

        self.unshuffle = nn.PixelUnshuffle(2)
        self.shuffle = nn.PixelShuffle(2)
        self.enc = RRDBEncoder(
            C=C,
            hid_S=hid_S,
            rrdb_encoder_num=rrdb_encoder_num,
            downsample_ratio=downsample_ratio,
        )
        self.filter = filter_block(
            downsample_scale=downsample_ratio, hid_S=hid_S, mat_size=self.mat_size
        )
        self.dec = RRDBDecoder(
            C=C,
            hid_S=hid_S,
            rrdb_decoder_num=rrdb_decoder_num,
            downsample_scale=downsample_ratio,
        )
        self.fuse = Compose(
            downsample_scale=downsample_ratio,
        ...
        self.enhance = ImageEnhancer(
            C_in=C,
            hid_S=hid_S,
            downsample_scale=downsample_ratio,
            rrdb_enhance_num=rrdb_enhance_num,
        )
    ...
    def forward(self, x_raw, **kwargs):
        ...
        compose_image, avg_image = self.fuse.feat_compose(
            image_list, [similarity_matrix[0]]
        )
...
class RRDBEncoder(nn.Module):
    def __init__(
        ...
        self.downsample_high = ResBlock(
            hid_S * (2 ** (self.scale_num - 2)),
            hid_S * (2 ** (self.scale_num - 1)),
            downsample=True,
            factor=downsample_ratio[-2],
        )
        self.downsample_low = ResBlock(
            hid_S * (2 ** (self.scale_num - 1)),
            hid_S * (2 ** (self.scale_num)),
            downsample=True,
            factor=downsample_ratio[-1],
        )
...
class filter_block(nn.Module):
    def __init__(self, downsample_scale, hid_S, mat_size):
        ...
        self.filter_block.append(
            nn.Sequential(
                nn.Conv2d(feat_len, hid_S, kernel_size=3, padding=1),
    ...
    def forward(self, x):
        ...
        for s in [-2, -1]:
            ...
            if mat_area != feat_area:
                out = F.interpolate(
                    x[s].clone(), size=tuple(self.mat_size[s]), mode="bilinear"
                )
            else:
                out = x[s].clone()
...
class MidMotionMatrix(nn.Module):
    ...
    def forward(self, x, B, T):
        ...
        pred_sim_matrix[1] = sim_matrix_interpolate(
            pred_fut_matrix.clone(), self.mat_size[0], self.mat_size[1]
        )
        ...
        for i in range(len(prev_sim_matrix)):
            new_cur_sim_matrix = torch.cat(
                [sim_matrix_postprocess(prev_sim_matrix[i]), pred_sim_matrix[i]], dim=1
            )
...
class Compose(nn.Module):
    ...
    def feat_generator(self, feats, sim_matrix, feat_idx, img_compose=False, scale=1):
        ...
        sim_matrix = sim_matrix.reshape(B, T * hw_cur, hw_target).permute(
            0, 2, 1
        )  # Batch, fut H*W, Prev T*HW
    ...
    def feat_compose(
        self, emb_feat_list, sim_matrix, img_compose=False, scale=1, use_gt=False
    ):
        """
        ...
        for i in range(len(emb_feat_list)):
            ...
            cur_emb_feat = (
                cur_emb_feat[:, : self.prev_len]
                if (not use_gt)
                else cur_emb_feat.clone()
            )

            for t in range(self.aft_seq_length):
                active_matrix_seq = sim_matrix_seq[:, : (self.prev_len - 1)]
                if t > 0:
                    fut_t_matrix = sim_matrix_seq[
                        :, (self.prev_len + t - 1) : (self.prev_len + t)
                    ]
                else:
                    fut_t_matrix = sim_matrix_seq[
                        :, (self.prev_len - 1) : (self.prev_len + t)
                    ]
                active_matrix_seq = torch.cat([active_matrix_seq, fut_t_matrix], dim=1)

                cur_sim_matrix = cum_multiply(
                    active_matrix_seq.clone()
                )  # B, T+1, h,w,h,w
                composed_t_feats = self.feat_generator(
                    cur_emb_feat[:, : self.prev_len].clone(),
                    cur_sim_matrix,
                    feat_idx=i,
                    img_compose=img_compose,
                    scale=scale,
                )
                ...
                if not use_gt:
                    if scale == 1:
                        if cur_emb_feat.shape[1] > self.prev_len + t:
                            cur_emb_feat[
                                :, t + self.prev_len
                            ] = composed_t_feats.clone()
                        else:
                            cur_emb_feat = torch.cat(
                                [cur_emb_feat, composed_t_feats.clone().unsqueeze(1)],
                                dim=1,
                            )  # cat compose features for next frame prediction

            temp = torch.stack(fut_emb_feat_list[i], dim=1)

            fut_emb_feat_list[i] = temp.reshape(
                -1, c, h * scale, w * scale
            )  # B*T,c,h,w
    ...
    def forward(self, x, similar_matrix, feat_shape):
        ...
        for i in range(len(x)):
            ...
            if i < len(x) - 2:
                h, w = x[i].shape[-2:]
                target_size = (
                    h // self.feat_scale_list[i] * self.feat_scale_list[i],
                    w // self.feat_scale_list[i] * self.feat_scale_list[i],
                )
                cur_feat = self.feat_unshuffle[i](
                    F.interpolate(x[i].clone(), size=target_size, mode="bilinear")
                )

                if (cur_feat.shape[-2] != self.mat_size[0][-2]) or (
                    cur_feat.shape[-1] != self.mat_size[0][-1]
                ):
                    compose_feat_list.append(
                        F.interpolate(
                            cur_feat, size=tuple(self.mat_size[0]), mode="bilinear"
                        )
                    )
                else:
                ...
                if (x[i].shape[-2] != self.mat_size[i - len(x) + 2][-2]) or (
                    x[i].shape[-1] != self.mat_size[i - len(x) + 2][-1]
                ):
                    compose_feat_list.append(
                        F.interpolate(
                            x[i],
                            size=tuple(self.mat_size[i - len(x) + 2]),
                            mode="bilinear",
                        )
                    )
                else:
        ...
        compose_fut_feat_list, _ = self.feat_compose(
            compose_feat_list, similar_matrix_for_compose
        )
        ...
        for i in range(len(compose_fut_feat_list)):
            if compose_fut_feat_list[i] is None:
                continue
            if i < len(x) - 2:
                compose_fut_feat_list[i] = self.feat_shuffle[i](
                    compose_fut_feat_list[i]
                )
            if (compose_fut_feat_list[i].shape[-2] != feat_shape[i][-2]) or (
                compose_fut_feat_list[i].shape[-1] != feat_shape[i][-1]
            ):
                compose_fut_feat_list[i] = F.interpolate(
                    compose_fut_feat_list[i], size=tuple(feat_shape[i]), mode="bilinear"
                )
    ...
    def __init__(
        ...
        for i in range(self.scale_num - 2, -1, -1):
            skip = (
                False
                if (
                    (i < self.scale_num - 1 and scale_in_use == 2)
                    or (i < self.scale_num - 2 and scale_in_use == 3)
                )
                else True
            )
...
class ImageEnhancer(nn.Module):
    def __init__(
        self, C_in=1, hid_S=32, downsample_scale=[2, 2, 2], rrdb_enhance_num=2
    ):
