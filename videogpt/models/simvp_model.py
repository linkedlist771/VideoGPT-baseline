...
class SimVP_Model(nn.Module):
    ...
    def forward(self, x_raw, **kwargs):
        ...
        _, C_, H_, W_ = embed.shape  # 10 x 64 x93 x126
        ...
        if clip_cond is not None:
            ...
            # Input clip_cond has shape [B, T, C_, H_, W_]
            B_clip, T_clip, C_clip, H_clip, W_clip = clip_cond.shape
            clip_cond_flat = clip_cond.reshape(B_clip * T_clip, C_clip, H_clip, W_clip)
            clip_cond_z = self.zero_conv(clip_cond_flat)
...
class Decoder(nn.Module):
    ...
    def forward(self, hid, enc1=None):
        for i in range(0, len(self.dec) - 1):  # 4个
            hid = self.dec[i](hid)
        Y = self.dec[-1](
            hid + enc1
        )  # hid: 10 x64 x372x504, enc1: 10x64x369x501  数据的形状： (156, 369, 501)
        Y = self.readout(Y)
        return Y
...
class MidIncepNet(nn.Module):
    ...
    def __init__(
        self, channel_in, channel_hid, N2, incep_ker=[3, 5, 7, 11], groups=8, **kwargs
    ):
...
class MetaBlock(nn.Module):
    ...
    def __init__(
        ...
        if model_type == "gsta":
            ...
            block_type = (
                "MHSA" if in_channels == out_channels and layer_i > 0 else "Conv"
            )
