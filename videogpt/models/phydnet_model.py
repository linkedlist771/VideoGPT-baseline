...
class PhyDNet_Model(nn.Module):
    ...
    def __init__(self, configs, **kwargs):
        ...
        self.encoder = PhyD_EncoderRNN(
            self.phycell, self.convcell, in_channel=C, patch_size=patch_size
        )
        self.k2m = K2M([7, 7])
    ...
    def forward(
        self, input_tensor, target_tensor, constraints, teacher_forcing_ratio=0.0
    ):
        loss = 0
        for ei in range(self.pre_seq_length - 1):
            _, _, output_image, _, _ = self.encoder(
    ...
    def inference(self, input_tensor, target_tensor, constraints, **kwargs):
        with torch.no_grad():
            loss = 0
            for ei in range(self.pre_seq_length - 1):
                encoder_output, encoder_hidden, output_image, _, _ = self.encoder(
                    input_tensor[:, ei, :, :, :], (ei == 0)
                )
                if kwargs.get("return_loss", True):
                    loss += self.criterion(
                        output_image, input_tensor[:, ei + 1, :, :, :]
                    )
