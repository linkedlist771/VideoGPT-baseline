...
class VideoSimVP(pl.LightningModule):
    def __init__(self, args):
        ...
        self.simvp = SimVP_Model(
            in_shape=(args.n_cond_frames, self.vqvae.embedding_dim, 
                     self.latent_shape[1], self.latent_shape[2]),
            hid_S=args.hid_S,
            hid_T=args.hid_T,
            N_S=args.N_S,
            N_T=args.N_T,
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            embeddings = embeddings.reshape(B, T, -1, self.latent_shape[1], self.latent_shape[2])
    ...
    def compute_loss(self, batch):
        ...
        if n_pred_frames <= self.args.n_cond_frames:
            target_frames = x[:, :, self.args.n_cond_frames:self.args.n_cond_frames + n_pred_frames]
        else:
    ...
    def sample(self, n, batch=None):
        ...
        with torch.no_grad():
            ...
            cond_embeddings = cond_embeddings.reshape(B, T, -1, self.latent_shape[1], self.latent_shape[2])
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--n_pred_frames", type=int, default=None,
