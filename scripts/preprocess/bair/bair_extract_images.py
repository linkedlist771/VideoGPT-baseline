...
from tensorflow.python.platform import flags, gfile

parser = argparse.ArgumentParser()
parser.add_argument(
    "--data_dir", default="", help="base directory to save processed data"
)
...
def convert_data(dname):
    ...
    while True:
        ...
        f = f.split("/")[-1]
        os.makedirs(
            "%s/processed_data/%s/%s/%d/" % (opt.data_dir, dname, f[:-10], k),
            exist_ok=True,
        )
        for i in range(len(seq)):
            imageio.imwrite(
                "%s/processed_data/%s/%s/%d/%d.png"
                % (opt.data_dir, dname, f[:-10], k, i),
                seq[i],
            )
        np.save(
            "%s/processed_data/%s/%s/%d/actions.npy"
            % (opt.data_dir, dname, f[:-10], k),
            actions,
        )
