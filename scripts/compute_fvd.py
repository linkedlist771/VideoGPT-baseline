...
from videogpt.fvd.fvd import frechet_distance, get_fvd_logits
    ...
    for _ in range(n_trials):
        ...
        if is_root:
            ...
            pbar.set_description(
                f"FVD {fvd_mean:.2f} +/- {fvd_std:.2f}, FVD* {fvd_star_mean:.2f} +/0 {fvd_star_std:.2f}"
            )
    if is_root:
        pbar.close()
        print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_trials", type=int, default=1, help="Number of trials to compute mean/std"
    )
